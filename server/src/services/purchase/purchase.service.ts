...
export const validateCardService = async (userId: TUserId['_id']) => {
    const cart = await validateCartInternalService(userId)

    return cart
}

export const checkOutService = async (userId: TUserId['_id']) => {
    const cart = await validateCartInternalService(userId)
...
export const getPurchasesService = async (userId: TUserId['_id']) => {
